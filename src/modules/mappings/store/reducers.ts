...
export const mappingReducer = (state: IMappingState, action: MappingsAction): IMappingState => {
  switch (action.type) {
  ...
  switch (action.type) {
    ...
    case MappingsActionTypes.DELETE_MAPPING_REQUEST:
      return {
        ...
        byId: {
          ...state.byId,
          [action.payload.mappingId]: mappingReducer(state.byId[action.payload.mappingId], action),
        },
      };
  ...
  switch (action.type) {
    case ServersActionTypes.INIT_SERVERS:
      return action.payload.servers.reduce(
        (agg: IMappingsState, server: IServer) => ({
          ...agg,
          [server.name]: {
            isLoading: false,
            haveBeenLoaded: false,
    ...
    case ServersActionTypes.CREATE_SERVER:
      return {
        ...state,
        [action.payload.server.name]: mappingsByServerReducer(undefined, action),
      };
