...
export const getMappings = (server: IServer) => ajax.getJSON(buildApiUrl(server, "/mappings"));
...
export const createMapping = (server: IServer, mapping: Partial<IMapping>) =>
  ajax.post(buildApiUrl(server, "/mappings"), mapping, { "Content-Type": "application/json" });

export const updateMapping = (server: IServer, mapping: IMapping) =>
  ajax.put(buildApiUrl(server, `/mappings/${mapping.id}`), mapping, {
