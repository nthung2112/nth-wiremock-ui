...
import { initCreateMapping, createMappingRequest, cancelMappingCreation } from "../store";
...
const mapStateToProps = (
  ...
  { serverName, creationId }: IOwnProps
): IPropsFromState => {
...
const mapDispatchToProps = (dispatch: Dispatch, props: IOwnProps) => ({
  init: () => {
    dispatch(initCreateMapping(props.serverName, props.creationId));
  },
  save: (mapping: IMapping) => {
    dispatch(createMappingRequest(props.serverName, props.creationId, mapping));
  },
  cancel: () => {
    dispatch(cancelMappingCreation(props.serverName, props.creationId));
  },
});
