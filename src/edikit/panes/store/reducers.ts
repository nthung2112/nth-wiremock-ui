...
const byNamespace = <Data>(
  ...
  switch (action.type) {
    ...
    case PanesActionTypes.SET_CURRENT_PANE:
      return {
        panes: setCurrentPane<Data>(state.panes, action.payload.paneId),
      };

    case PanesActionTypes.ADD_CONTENT_TO_CURRENT_PANE:
      return {
        panes: addContentToCurrentPane<Data>(state.panes, action.payload.content),
      };
    ...
    case PanesActionTypes.REMOVE_CONTENT_FROM_ALL_PANES:
      return {
        panes: removeContentFromAllPanes<Data>(state.panes, action.payload.contentId),
      };

    case PanesActionTypes.SPLIT_PANE:
      return {
        panes: splitPane<Data>(state.panes, action.payload.paneId, action.payload.axis),
      };
...
export const panesReducer =
  <Data>() =>
  (state: IPanesState<Data> = {}, action: PanesAction<Data>): IPanesState<Data> => {
    switch (action.type) {
      case PanesActionTypes.INIT_PANES_NAMESPACE:
      case PanesActionTypes.SET_CURRENT_PANE:
      case PanesActionTypes.ADD_CONTENT_TO_CURRENT_PANE:
      ...
      case PanesActionTypes.SPLIT_PANE:
        return {
          ...state,
          [action.payload.namespace]: byNamespace(state[action.payload.namespace], action),
        };
